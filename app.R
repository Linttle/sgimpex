        ...
        tabItems(
            ...
            tabItem(tabName = "MAGICQUADRANT",
                    fluidRow(
                        ...
                        column(2,
                               sliderInput(
                                   inputId = "magicQuadrantFilter",
                                   label = "Year",
                                   min = min(magicquadrantdata$year),
                                   max = max(magicquadrantdata$year),
                                   value = max(magicquadrantdata$year),
                                   step = 1,
...
server <- function(input, output) {
    ...
    output$magicQuadrant <- renderPlotly({
        magicquadrantdataplot <- filter(magicquadrantdata, year == input$magicQuadrantFilter)
        plot <- ggplot(magicquadrantdataplot, aes(x=export_percentile, y=import_percentile, color=trade_balance, text= paste0("Country: ", country))) +
